...
async function main() {
    ...
    // Variables to change
    const DELAY = 11;
    const NEW_REWARD_PER_BLOCK = ethers.utils.parseEther("120");

    // Do not change this line
    const iface = new Interface(["function setRewardsPerBlock(uint)"]);
    const DATA = iface.encodeFunctionData("setRewardsPerBlock", [NEW_REWARD_PER_BLOCK.toString()]);

    let tx = await Contract.connect(signer).schedule(stabilityPoolAddress, value, DATA, zeroBytes, salt, DELAY);
    await tx.wait();
...
/*
ENV=demo npx hardhat run scripts/timeLock/rewardPerBlockSchedule.js --network bscTestnet
ENV=oracle npx hardhat run scripts/timeLock/rewardPerBlockSchedule.js --network bscTestnet
*/